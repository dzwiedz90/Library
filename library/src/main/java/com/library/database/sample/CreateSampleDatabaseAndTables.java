...
import com.library.database.MySQLConnector;
...
public class CreateSampleDatabaseAndTables {
	...
	private void createPublishersTable() throws SQLException {
		statement = connection.createStatement();
		statement.executeUpdate("CREATE TABLE publishers(publishers_id INT AUTO_INCREMENT PRIMARY KEY NOT NULL, publishers_name VARCHAR(64) NOT NULL, publishers_year_established INT(4), cities_id INT NOT NULL, FOREIGN KEY (cities_id) REFERENCES cities(cities_id));");
	}

	private void createAuthorsTable() throws SQLException {
		statement = connection.createStatement();
		statement.executeUpdate("CREATE TABLE authors(authors_id INT AUTO_INCREMENT PRIMARY KEY NOT NULL, authors_name VARCHAR(32) NOT NULL, authors_surname VARCHAR(20) NOT NULL, authors_biography TEXT NOT NULL, authors_year_born INT(4) NOT NULL, authors_year_died INT(4));");
	}
	
	private void createReadersTable() throws SQLException {
		statement = connection.createStatement();
		statement.executeUpdate("CREATE TABLE readers(readers_id INT AUTO_INCREMENT PRIMARY KEY NOT NULL, readers_name VARCHAR(32) NOT NULL, readers_surname VARCHAR(20) NOT NULL, readers_date_of_birth VARCHAR(10) NOT NULL, readers_address VARCHAR(32) NOT NULL, readers_joined VARCHAR(10) NOT NULL, cities_id INT NOT NULL, FOREIGN KEY (cities_id) REFERENCES cities(cities_id));");
	}

	private void createBooksTable() throws SQLException {
		statement = connection.createStatement();
		statement.executeUpdate("CREATE TABLE books(books_id INT AUTO_INCREMENT PRIMARY KEY NOT NULL, books_title VARCHAR(64) NOT NULL, books_isbn INT(13) NOT NULL, books_year_published INT(4) NOT NULL, authors_id INT NOT NULL, publishers_id INT NOT NULL, FOREIGN KEY (authors_id) REFERENCES authors(authors_id), FOREIGN KEY (publishers_id) REFERENCES publishers(publishers_id));");
	}
